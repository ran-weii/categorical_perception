...
class FIRL(Model):
    """ FIRL with reverse kl minimization """
    def __init__(
        self, agent, hidden_dim, num_hidden, activation, gamma=0.9, beta=0.2, polyak=0.995, norm_obs=False,
        buffer_size=int(1e6), d_batch_size=100, a_batch_size=32, rnn_len=50, reward_steps=500, d_steps=50, a_steps=50, 
        ...
        Args:
            ...
            reward_steps (int, optional): number of episode steps to compute reward loss. Default=500
    ...
    def __repr__(self):
        ...
        s = "{}(gamma={}, beta={}, polyak={}, norm_obs={}, "\
            ...
            self.lr, self.decay, self.grad_clip, self.grad_penalty, 
            s_discriminator, s_reward, s_agent, s_critic
        )
        return s
    ...
    def compute_discriminator_loss(self): 
        ...
        inputs = torch.cat([real_obs_norm, fake_obs_norm], dim=0)
        ...
        gp = self.gradient_penalty(real_obs_norm, fake_obs_norm)
        return d_loss, gp
    
    def compute_reward_loss(self):
        real_batch = self.real_buffer.sample_episodes(self.a_batch_size, self.reward_steps, prioritize=False)
        fake_batch = self.replay_buffer.sample_episodes(self.a_batch_size, self.reward_steps, prioritize=True)
        ...
        # compute cumulative log density ratio
        with torch.no_grad():
            real_log_r = torch.sum(self.discriminator(real_obs_norm).squeeze(-1) * real_mask, dim=0)
            fake_log_r = torch.sum(self.discriminator(fake_obs_norm).squeeze(-1) * fake_mask, dim=0)
            log_r = torch.cat([real_log_r, fake_log_r], dim=0)

        # compute cumulative reward
        real_reward = torch.sum(self.compute_reward(real_obs_norm).squeeze(-1) * real_mask, dim=0)
        fake_reward = torch.sum(self.compute_reward(fake_obs_norm).squeeze(-1) * fake_mask, dim=0)
        reward = torch.cat([real_reward, fake_reward], dim=0)
        
        # compute covariance
        r_loss = torch.mean(log_r * reward) - torch.mean(log_r) * torch.mean(reward)
        return r_loss / self.reward_steps
    ...
    def compute_critic_loss(self):
        ...
        with torch.no_grad():    
            # compute reward
            r = self.compute_reward(obs_norm)
    ...
    def take_gradient_step(self, logger=None):
        ...
        for i in range(self.d_steps):
            ...
            r_loss_epoch.append(r_loss.data.item())
            
            if logger is not None:
                logger.push({"r_loss": r_loss.data.item()})
